...
double mgcl_vcycle(mgcl_config *conf, mgcl_level_data *data, int level)
{
    ...
    // relax nu1 times
    if (conf->stencil_values)
        res = mgcl_stencil_jacobi(conf, &data[level], conf->nu1, !conf->ignoreTol);
    else
        res = mgcl_jacobi(conf, &data[level], conf->nu1, !conf->ignoreTol);
    ...
    // relax nu2 times
    if (conf->stencil_values)
        res = mgcl_stencil_jacobi(conf, &data[level], conf->nu2, !conf->ignoreTol);
    else
        res = mgcl_jacobi(conf, &data[level], conf->nu2, !conf->ignoreTol);
...
void mgcl_c_mgcl(mgcl_config *conf)
{
    ...
    for (int i = 0; i < conf->maxiter_vcycles; i++)
    {
        ...
        if (!conf->silent)
            printf("iter = %d, elapsed time = %2.5lf s, rel. res = %e\n", i, tend - tstart, relres);

        if (!conf->ignoreTol && relres < conf->tol)
            break;
    }
