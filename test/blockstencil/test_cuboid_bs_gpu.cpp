...
// Check if CuboidBSGpu gets initialized correctly with host_ptr being null.
TEST_CASE("CuboidBSGpu ctor no host_data", "[ocl]")
{
    int blocksize = 2;
    ...
    SECTION("success")
    {
        ...
        mgcl::CuboidBSGpu c(tu.getContext(), CL_MEM_READ_WRITE, 1, 2, 3, 2, 3, 4, blocksize);
        ...
        REQUIRE(c.getSize() == c.getMgh() * c.getNgh() * c.getOgh() * blocksize);
    }

    SECTION("Invalid dimensions")
    {
        mgcl_test::TestUtility tu(deviceType);
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, 0, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, 1, 0, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, 1, 2, 0, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, 1, 2, 3, -1, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, 1, 2, 3, 2, -1, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, 1, 2, 3, 2, 3, -1, blocksize));
    }

    SECTION("invalid flags")
    {
        mgcl::CuboidBS ch(1, 2, 3, 2, 3, 4, blocksize);
        mgcl_test::TestUtility tu(deviceType);

        // flags must contain one of CL_MEM_READ_WRITE, CL_MEM_WRITE_ONLY or CL_MEM_READ_ONLY.
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_COPY_HOST_PTR, 1, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_USE_HOST_PTR, 1, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_ALLOC_HOST_PTR, 1, 2, 3, 2, 3, 4, blocksize));

        // flags must not not contain CL_MEM_COPY_HOST_PTR, CL_MEM_USE_HOST_PTR or CL_MEM_ALLOC_HOST_PTR
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE, 1, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, 1, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_WRITE, 1, 2, 3, 2, 3, 4, blocksize));

        // flags must not contain multiple of CL_MEM_READ_WRITE, CL_MEM_WRITE_ONLY or CL_MEM_WRITE_ONLY
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE | CL_MEM_WRITE_ONLY, 1, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE | CL_MEM_READ_ONLY, 1, 2, 3, 2, 3, 4, blocksize));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_WRITE_ONLY | CL_MEM_READ_ONLY, 1, 2, 3, 2, 3, 4, blocksize));
    }
}

// Check if CuboidBSGpu gets initialized correctly with host_ptr not being null.
TEST_CASE("CuboidBSGpu ctor host_data given", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    SECTION("sucess")
    {
        ...
        mgcl_test::TestUtility tu(deviceType);
        mgcl::CuboidBSGpu c(tu.getContext(), CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, ch);
    ...
    SECTION("invalid flags")
    {
        ...
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_WRITE_ONLY, ch));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_ONLY, ch));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_READ_WRITE, ch));

        // flags must not contain multiple of CL_MEM_COPY_HOST_PTR, CL_MEM_USE_HOST_PTR or CL_MEM_ALLOC_HOST_PTR if
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_COPY_HOST_PTR | CL_MEM_USE_HOST_PTR, ch));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_COPY_HOST_PTR | CL_MEM_ALLOC_HOST_PTR, ch));
        REQUIRE_THROWS(mgcl::CuboidBSGpu(tu.getContext(), CL_MEM_USE_HOST_PTR | CL_MEM_ALLOC_HOST_PTR, ch));
    }
}

// Check if CuboidBSGpu gets initialized correctly retaining an existing buffer.
TEST_CASE("CuboidBSGpu ctor retaining buffer", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    SECTION("success")
    {
        ...
        mgcl::CuboidBSGpu* c = new mgcl::CuboidBSGpu(tu.getContext(), 0, 1, 2, 3, 2, 3, 4, blocksize, buf);
...
TEST_CASE("CuboidBSGpu::read into new", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
...
TEST_CASE("CuboidBSGpu::read into existing", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    mgcl::CuboidBS ch_act(1, 2, 3, 2, 3, 4, blocksize);
    c.read(tu.getCommands(), &ch_act, true);

    REQUIRE(ch_act.isEqualAllCells(ch));
}

TEST_CASE("CuboidBSGpu::write", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
...
TEST_CASE("CuboidBSGpu::fill", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
...
// Tests if CuboidBSGpu::copyTo works correctly.
TEST_CASE("CuboidBSGpu::copyTo", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    SECTION("success")
    {
        ...
        mgcl::CuboidBSGpu c2(tu.getContext(), CL_MEM_READ_WRITE, 1, 2, 3, 2, 3, 4, blocksize);
    ...
    SECTION("throwing")
    {
        ...
        mgcl::CuboidBSGpu c2(tu.getContext(), CL_MEM_READ_WRITE, 3, 2, 3, 2, 3, 4, blocksize);
        REQUIRE_THROWS(c.copyTo(tu.getCommands(), c2));
    }
}

// Tests if CuboidBSGpu::swap works correctly.
TEST_CASE("CuboidBSGpu::swap", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    SECTION("throwing")
    {
        mgcl::CuboidBSGpu c1(tu.getContext(), CL_MEM_READ_WRITE, 3, 1, 1, 0, 0, 0, blocksize);
        mgcl::CuboidBSGpu c2(tu.getContext(), CL_MEM_READ_WRITE, 1, 1, 1, 0, 0, 0, blocksize);
        REQUIRE_THROWS(mgcl::CuboidBSGpu::swap(c1, c2));
    }
}

// Tests if CuboidBSGpu::cloneShallow works correctly.
TEST_CASE("CuboidBSGpu::copyShallow", "[ocl]")
{
    int blocksize = 2;
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    SECTION("success")
    {
        ...
        mgcl::CuboidBSGpu c_ro(tu.getContext(), CL_MEM_READ_ONLY, 1, 2, 3, 1, 2, 3, blocksize);
        mgcl::CuboidBSGpu c_wo(tu.getContext(), CL_MEM_WRITE_ONLY, 1, 2, 3, 1, 2, 3, blocksize);
        ...
        REQUIRE(c_ro.getBlocksize() == c2_ro.getBlocksize());
        ...
        REQUIRE(c_wo.getBlocksize() == c2_wo.getBlocksize());
...
//         mgcl::CuboidBS h_planes(1, 1, ressize);
...
//         mgcl::CuboidBSGpu d_cuboid(p.getContext(), CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, h_cuboid);
