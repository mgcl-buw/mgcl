...
// Also uses different amounts of jacobi iterations without ghost update in-between.
// StencilValues on level 1 on rank 0 must have halve of global sizes.
// Run with e.g. mpiexec -n 2 tests_mpi MPI_vcycle_GPU_overlapped_Jacobi_threshold_eq_2_Varying27p_multiple_jacobi_iters
TEST_CASE("MPI_vcycle_GPU_overlapped_Jacobi_threshold_eq_2_Varying27p_multiple_jacobi_iters")
{
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
...
// Run with e.g. mpiexec -n 2 tests_mpi MPI_vcycle_multiple_solve_calls
TEST_CASE("MPI_vcycle_multiple_solve_calls")
{
    using std::min;
    ...
    std::cout << "Seq First call on rank " << mpi_rank << " ..." << std::endl;
    p_seq.solve();
    MPI_Barrier(mpi_comm);
    ...
    std::cout << "OCL First call on rank " << mpi_rank << " ..." << std::endl;
    p.solve();
    MPI_Barrier(mpi_comm);
...
// Tests if vcycle is correct for multiple processes using OCL with mpiLevelThreshold = 1, so
// stencil values are gathered for level 1 from level 0, which is a special case regarding the setup.
// In addition, coarse levels are calculated on the host, while fine levels are calculated on the GPU.
// Run with e.g. mpiexec -n 2 tests_mpi MPI_vcycle_GPU_threshold_eq_1_Varying27p_maxLevelUsingOcl
TEST_CASE("MPI_vcycle_GPU_threshold_eq_1_Varying27p_maxLevelUsingOcl")
{
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    int maxLevelUsingOcl = GENERATE_COPY(range(0, maxlv)); // from 0 to maxlv-1
    CAPTURE(maxLevelUsingOcl, maxlv, m);
    ...
    else
    {
        ...
        std::cout << "ocl MPI threshold=1, maxLevelUsingGpu=" << std::to_string(maxLevelUsingOcl) << ", Varying27p" << std::endl;
        std::cout << "rank 0: " << std::endl;
        std::cout
            << std::scientific << std::setprecision(17) << "  ||e||_2 = " << errNorm << std::endl
            << std::scientific << std::setprecision(17) << "  e_max = " << errMax << std::endl;
...
// StencilValues on level 1 on rank 0 must have halve of global sizes.
// In addition, coarse levels are calculated on the host, while fine levels are calculated on the GPU.
// Run with e.g. mpiexec -n 2 tests_mpi MPI_vcycle_GPU_threshold_eq_2_Varying27p_maxLevelUsingOcl
TEST_CASE("MPI_vcycle_GPU_threshold_eq_2_Varying27p_maxLevelUsingOcl")
{
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    else
    {
        ...
        std::cout << "ocl MPI threshold=2, maxLevelUsingGpu=" << std::to_string(maxLevelUsingOcl) << ", Varying27p" << std::endl;
        std::cout << "rank 0: " << std::endl;
        std::cout
            << std::scientific << std::setprecision(17) << "  ||e||_2 = " << errNorm << std::endl
            << std::scientific << std::setprecision(17) << "  e_max = " << errMax << std::endl;
...
// Tests if vcycle is correct for multiple processes with mpiLevelThreshold = 2, so stencil values are gathered
// for level 2 from level 1. StencilValues on level 1 on rank 0 must have halve of global sizes. Uses
// an anisotropic problem with Dirichlet bc.
// Run with e.g. mpiexec -n 2 tests_mpi MPI_vcycle_threshold_eq_2_Varying27p_anisotropic
TEST_CASE("MPI_vcycle_threshold_eq_2_Varying27p_anisotropic")
{
    using std::min;
    ...
    auto f = std::make_shared<mgcl::Cuboid>(m, n, o, ghin, ghin, ghin);
    auto solution = std::make_shared<mgcl::Cuboid>(m, n, o, ghin, ghin, ghin);
    auto svglob = std::make_shared<mgcl::VaryingStencil>(m, n, o, 3, ghin, ghin, ghin);
    int dummy[3] = {0, 0, 0}; // use dummy for mpicoords to create global problem
    mgcl_test::createAnisotropicProblem(*v, *f, *solution, *svglob, dummy);

    // Create local slices
    std::shared_ptr<mgcl::Cuboid> vloc(v->slice(m_start, m_end, n_start, n_end, o_start, o_end));
    std::shared_ptr<mgcl::Cuboid> floc(f->slice(m_start, m_end, n_start, n_end, o_start, o_end));
    std::shared_ptr<mgcl::Cuboid> solutionloc(solution->slice(m_start, m_end, n_start, n_end, o_start, o_end));
    std::shared_ptr<mgcl::VaryingStencil> svloc(svglob->slice(m_start, m_end, n_start, n_end, o_start, o_end));
    ...
    REQUIRE(svloc->getM() == sv.getM());
    REQUIRE(svloc->getN() == sv.getN());
    REQUIRE(svloc->getO() == sv.getO());
    REQUIRE(svloc->getMgh() == sv.getMgh());
    REQUIRE(svloc->getNgh() == sv.getNgh());
    REQUIRE(svloc->getOgh() == sv.getOgh());
    REQUIRE(svloc->getSize() == sv.getSize());

    for (size_t i = 0; i < svloc->getSize(); i++)
    {
        sv.field1d()[i] = svloc->field1d()[i];
    }
    ...
    else
    {
        ...
        // check if solution is good
        auto err = calculateError(*solution, *v);
        auto errNorm = calculateErrorNorm(1.0 / (double)(m + 1), *err);
        auto errMax = calculateMaxError(*err);

        // err->dumpToFile("err.txt");

        std::cout << "seq MPI anisotropic Varying27p" << std::endl;
        std::cout << "rank 0: " << std::endl;
        std::cout
            << std::scientific << std::setprecision(17) << "  ||e||_2 = " << errNorm << std::endl
            << std::scientific << std::setprecision(17) << "  e_max = " << errMax << std::endl;
...
// an anisotropic problem with Dirichlet bc.
// Run with e.g. mpiexec -n 2 tests_mpi MPI_vcycle_threshold_eq_2_Varying27p_anisotropic
TEST_CASE("MPI_vcycle_GPU_threshold_eq_2_Varying27p_anisotropic")
{
    using std::min;
...
{
    double max = 0;
    for (int i = error.getGhostsM(); i < error.getM() + error.getGhostsM(); i++)
        for (int j = error.getGhostsN(); j < error.getN() + error.getGhostsN(); j++)
            for (int k = error.getGhostsO(); k < error.getO() + error.getGhostsO(); k++)
            {
                if (max < error[i][j][k])
                    max = error[i][j][k];
            }
