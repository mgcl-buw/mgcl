...
TEST_CASE("MPI_jacobi_ocl_Laplace_n_processes", "[mpiN]")
{
    ...
    p_glob.setDeviceType(deviceType);
    ...
    for (int il = v_glob.getGhostsM(), ig = mpi_coords[0] * ml + v_loc.getGhostsM(); il < v_loc.getM() + v_glob.getGhostsM(); il++, ig++)
        for (int jl = v_glob.getGhostsN(), jg = mpi_coords[1] * nl + v_loc.getGhostsN(); jl < v_loc.getN() + v_glob.getGhostsN(); jl++, jg++)
            ...
            {
                CAPTURE(il, jl, kl, ig, jg, kg);
                REQUIRE_THAT(v_loc[il][jl][kl], Catch::Matchers::WithinAbs(v_glob[ig][jg][kg], 1e-14));
                REQUIRE_THAT(f_loc[il][jl][kl], Catch::Matchers::WithinAbs(f_glob[ig][jg][kg], 1e-14));
                REQUIRE_THAT(f_loc_ret[il][jl][kl], Catch::Matchers::WithinAbs(f_loc[il][jl][kl], 1e-14));
                REQUIRE_THAT(f_loc_ret[il][jl][kl], Catch::Matchers::WithinAbs(f_glob[ig][jg][kg], 1e-14));
            }
    ...
    for (int il = v_glob.getGhostsM(), ig = mpi_coords[0] * ml + v_loc.getGhostsM(); il < v_loc.getM() + v_glob.getGhostsM(); il++, ig++)
        for (int jl = v_glob.getGhostsN(), jg = mpi_coords[1] * nl + v_loc.getGhostsN(); jl < v_loc.getN() + v_glob.getGhostsN(); jl++, jg++)
            ...
            {
                CAPTURE(il, jl, kl, ig, jg, kg);
                REQUIRE_THAT(v_loc_ret[il][jl][kl], Catch::Matchers::WithinAbs(v_glob_ret[ig][jg][kg], 1e-14));
            }
}
...
// Run with: mpiexec -n 8 tests_mpi "MPI_jacobi_ocl_VaryingStencil_n_processes"
// TODO non-periodic
TEST_CASE("MPI_jacobiOverlapped_ocl_VaryingStencil_n_processes")
{
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    p.setStencilType(stencilType);
    p.setResidualNorm(resnorm);
    p.setOverlappedJacobiGhostUpdateMaxLevel(0); // use overlapped Jacobi ghost update from level 0. This is needed to make mgcl create a second command queue.
    ...
    mgcl::MultigridEngine::jacobi(p_glob, lv_glob, maxiter, true, stepsPerIter);
    mgcl::MultigridEngine::jacobiOverlapped(p, lv, maxiter, true, stepsPerIter, p.getCommands2());
    p.getOpenCLHelper().finish();
    p_glob.getOpenCLHelper().finish();
...
// Use an arbitrary stencil with no duplicate values (result is only checked against global jacobi, not
// against the actual solution).
// Run with: mpiexec -n 8 tests_mpi "MPI_jacobiOverlapped_ocl_LaplaceStencils_n_processes"
// TODO non-periodic
TEST_CASE("MPI_jacobiOverlapped_ocl_LaplaceStencils_n_processes")
{
    auto deviceType = GENERATE(mgcl_test::deviceTypes(CLI_ARGS::deviceTypes));
    ...
    // v_loc_ret.dumpToFile("v_loc_ret_" + std::to_string(mpi_rank) + ".txt");
    // if (mpi_rank == 0)
    //     v_glob_ret.dumpToFile("v_glob_ret.txt");
