...
/* Restricts non-fixed stencil from fine to coarse grid.
 * Needs to get called with m*n*o work-items, o excluding stencil size.
 * m,n,o is size of ghosted coarse grid, o including stencil size.
 * fine and coarse must be of sizes of ghosted grids.
 * TODO proper iomplementation */
__kernel void stencil_restrict_to_coarse(__global double *restrict fine, __global double *restrict coarse, const int m,
                                         const int n, const int o, const int ghosts, const int stencil_size_multiplier)
{
    int i = get_global_id(0);
    int j = get_global_id(1);
    int k = get_global_id(2);
    ...
    if (i < m && j < n && k < o)
    { // TODO check indices
        const int index = (i + ghosts) * n * o + (j + ghosts) * o + (k + ghosts) * stencil_size_multiplier;
        const int nf = (n - g2) * 2 + g2, of = (o - g2 * stencil_size_multiplier) * 2 + g2 * stencil_size_multiplier;
        const int i2 = i * 2 + ghosts + 1, j2 = j * 2 + ghosts + 1;
        int k2;

        // restrict each stencil value for current grid point
        for (int st = 0; st < stencil_size_multiplier; st++)
        {
            k2 = (k * 2 + ghosts + 1) * stencil_size_multiplier + st;
            coarse[index + st] =
                0.125 * fine[i2 * nf * of + j2 * of + k2] // self
                                                          // direct neighbours
                + 0.0625 * fine[(i2 - 1) * nf * of + j2 * of + k2] + 0.0625 * fine[(i2 + 1) * nf * of + j2 * of + k2] +
                0.0625 * fine[i2 * nf * of + (j2 - 1) * of + k2] + 0.0625 * fine[i2 * nf * of + (j2 + 1) * of + k2] +
                0.0625 * fine[i2 * nf * of + j2 * of + k2 - stencil_size_multiplier] +
                0.0625 * fine[i2 * nf * of + j2 * of + k2 + stencil_size_multiplier]
                // edge midpoints xy-plane
                + 0.03125 * fine[(i2 - 1) * nf * of + (j2 - 1) * of + k2] +
                0.03125 * fine[(i2 - 1) * nf * of + (j2 + 1) * of + k2] +
                ...
                0.03125 * fine[(i2 + 1) * nf * of + (j2 + 1) * of + k2]
                // edge midpoints xz-plane
                + 0.03125 * fine[(i2 - 1) * nf * of + j2 * of + k2 - stencil_size_multiplier] +
                0.03125 * fine[(i2 - 1) * nf * of + j2 * of + k2 + stencil_size_multiplier] +
                0.03125 * fine[(i2 + 1) * nf * of + j2 * of + k2 - stencil_size_multiplier] +
                0.03125 * fine[(i2 + 1) * nf * of + j2 * of + k2 + stencil_size_multiplier]
                // edge midpoints yz-plane
                + 0.03125 * fine[i2 * nf * of + (j2 - 1) * of + k2 - stencil_size_multiplier] +
                0.03125 * fine[i2 * nf * of + (j2 - 1) * of + k2 + stencil_size_multiplier] +
                0.03125 * fine[i2 * nf * of + (j2 + 1) * of + k2 - stencil_size_multiplier] +
                0.03125 * fine[i2 * nf * of + (j2 + 1) * of + k2 + stencil_size_multiplier]
                // corners
                + 0.015625 * fine[(i2 - 1) * nf * of + (j2 - 1) * of + k2 - stencil_size_multiplier] +
                0.015625 * fine[(i2 - 1) * nf * of + (j2 - 1) * of + k2 + stencil_size_multiplier] +
                0.015625 * fine[(i2 - 1) * nf * of + (j2 + 1) * of + k2 - stencil_size_multiplier] +
                0.015625 * fine[(i2 - 1) * nf * of + (j2 + 1) * of + k2 + stencil_size_multiplier] +
                0.015625 * fine[(i2 + 1) * nf * of + (j2 - 1) * of + k2 - stencil_size_multiplier] +
...
/* Prolongates non-fixed stencil from coarse to fine grid.
 * Needs to get called with m*n*o work-items, o excluding stencil size.
 * m,n,o is size of ghosted coarse grid, o including stencil size.
 * fine and coarse must be of sizes of ghosted grids.
 * TODO proper iomplementation */
__kernel void stencil_prolongate_to_fine(__global double *restrict fine, __global double *restrict coarse, const int m,
                                         const int n, const int o, const int ghosts, const int stencil_size_multiplier)
{
    int i = get_global_id(0);
    int j = get_global_id(1);
    ...
    const int mc = (m - g2) / 2 + g2, nc = (n - g2) / 2 + g2,
              oc = (o - g2 * stencil_size_multiplier) / 2 + g2 * stencil_size_multiplier;

    if (i > ghosts - 1 && i < mc - ghosts && j > ghosts - 1 && j < nc - ghosts && k > ghosts - 1 && k < oc - ghosts)
    {
        const int index_coarse = i * nc * oc + j * oc + k * stencil_size_multiplier;
        const int i2 = i * 2 - (ghosts - 1), j2 = j * 2 - (ghosts - 1);
        int k2;
        ...
        {
            k2 = k * 2 - (ghosts - 1) * stencil_size_multiplier + st;
            fine[i2 * n * o + j2 * o + k2] = coarse[index_coarse + st];

            fine[i2 * n * o + j2 * o + k2 - stencil_size_multiplier] =
                0.5 * (coarse[index_coarse + st] + coarse[index_coarse + st - 1]);
            fine[i2 * n * o + (j2 - 1) * o + k2] =
                0.5 * (coarse[index_coarse + st] + coarse[i * nc * oc + (j - 1) * oc + k]);
            fine[(i2 - 1) * n * o + j2 * o + k2] =
                0.5 * (coarse[index_coarse + st] + coarse[(i - 1) * nc * oc + j * oc + k]);

            fine[i2 * n * o + (j2 - 1) * o + k2 - stencil_size_multiplier] =
                0.25 * (coarse[index_coarse + st] + coarse[index_coarse + st - 1] +
                        coarse[i * nc * oc + (j - 1) * oc + k] + coarse[i * nc * oc + (j - 1) * oc + k - 1]);
            fine[(i2 - 1) * n * o + j2 * o + k2 - stencil_size_multiplier] =
                0.25 * (coarse[index_coarse + st] + coarse[index_coarse + st - 1] +
                        coarse[(i - 1) * nc * oc + j * oc + k] + coarse[(i - 1) * nc * oc + j * oc + k - 1]);
            fine[(i2 - 1) * n * o + (j2 - 1) * o + k2] =
                0.25 * (coarse[index_coarse + st] + coarse[i * nc * oc + (j - 1) * oc + k] +
                        coarse[(i - 1) * nc * oc + j * oc + k] + coarse[(i - 1) * nc * oc + (j - 1) * oc + k]);

            fine[(i2 - 1) * n * o + (j2 - 1) * o + k2 - stencil_size_multiplier] =
                0.125 *
                (coarse[index_coarse + st] + coarse[index_coarse + st - 1] + coarse[i * nc * oc + (j - 1) * oc + k] +
                 coarse[i * nc * oc + (j - 1) * oc + k - 1] + coarse[(i - 1) * nc * oc + j * oc + k] +
                 coarse[(i - 1) * nc * oc + j * oc + k - 1] + coarse[(i - 1) * nc * oc + (j - 1) * oc + k] +
                 coarse[(i - 1) * nc * oc + (j - 1) * oc + k - 1]);
        }
 ...
 * dinv is h2/A(i,i), e.g. h2/6.0 for 3D laplacian stencil
 * if store_residual is true, the residual will be stored into global field r */
__kernel void jacobi_stencil_iter_7point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict v_out, __global double *restrict f, __global double *restrict r,
    __global double *restrict stencil_values, const double h2inv, const double omega, const int m, const int n,
    const int o, const int ghosts, const int store_residual)
{
    int j = get_global_id(0);
    int k = get_global_id(1);
    ...
    if (j > ghosts - 1 && k > ghosts - 1 && j < n - ghosts && k < o - ghosts)
    {
        ...
        for (int i = ghosts; i < m - ghosts; i++)
        {
            ...
            // A*v
            double stencilsum = stencil_values[stencil_idx] * v_in_index +
                                stencil_values[stencil_idx + 1] * v_in[index - koff] +
                                stencil_values[stencil_idx + 1] * v_in[index + koff] +
                                stencil_values[stencil_idx + 2] * v_in[index - joff] +
                                stencil_values[stencil_idx + 2] * v_in[index + joff] +
                                stencil_values[stencil_idx + 3] * v_in[index - ioff] +
                                stencil_values[stencil_idx + 3] * v_in[index + ioff];
            ...
            // u_(m+1) = u_(m) + omega * (D^-1) * r_(m)
            v_out[index] = v_in_index + omega * (1.0 / stencil_values[stencil_idx]) * res;

            // if (get_global_id(0) == ghosts && get_global_id(1) == ghosts && i >= ghosts && i <= ghosts)
            // {
            //     // printf("x = %d, res = %e, v_out = %e\n", i, res, v_out[index]);
            //     print_7point(v_in, index, ioff, joff, koff);
            //     printf("stencilsum = %e, vout = %e\n", stencilsum, v_out[index]);
            //     // printf("stencil_values: %f, %f, %f, %f\n", stencil_values[stencil_idx],
            //     stencil_values[stencil_idx+1], stencil_values[stencil_idx+2], stencil_values[stencil_idx+3]);
            // }
 ...
 * dinv is h2/A(i,i), e.g. h2/6.0 for 3D laplacian stencil
 * if store_residual is true, the residual will be stored into global field r */
__kernel void jacobi_stencil_iter_19point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict v_out, __global double *restrict f, __global double *restrict r,
    __global double *restrict stencil_values, const double h2inv, const double omega, const int m, const int n,
    const int o, const int ghosts, const int store_residual)
{
    ...
    if (j > ghosts - 1 && k > ghosts - 1 && j < n - ghosts && k < o - ghosts)
    {
        ...
        int koff = 1;
        int index = ghosts * ioff + j * o + k;
        int stencil_idx = 7 * index;
        int stencil_ioff = 7 * ioff;
        ...
        for (int i = ghosts; i < m - ghosts; i++)
        {
            ...
            double stencilsum = stencil_values[stencil_idx] * v_in_index +
                                ...
                                - stencil_values[stencil_idx + 4] * v_in[index - joff - koff] -
                                stencil_values[stencil_idx + 4] * v_in[index - joff + koff] -
                                stencil_values[stencil_idx + 4] * v_in[index + joff - koff] -
                                stencil_values[stencil_idx + 4] * v_in[index + joff + koff] -
                                stencil_values[stencil_idx + 6] * v_in[index - ioff - koff] -
                                stencil_values[stencil_idx + 6] * v_in[index - ioff + koff] -
                                stencil_values[stencil_idx + 6] * v_in[index + ioff - koff] -
                                stencil_values[stencil_idx + 6] * v_in[index + ioff + koff] -
                                stencil_values[stencil_idx + 5] * v_in[index - ioff - joff] -
                                stencil_values[stencil_idx + 5] * v_in[index - ioff + joff] -
                                stencil_values[stencil_idx + 5] * v_in[index + ioff - joff] -
                                stencil_values[stencil_idx + 5] * v_in[index + ioff + joff];
            ...
            // u_(m+1) = u_(m) + omega * (D^-1) * r_(m)
            v_out[index] = v_in_index + omega * res * (1.0 / stencil_values[stencil_idx]);

            if (store_residual)
                r[index] = res;

            index += ioff;
            stencil_idx += stencil_ioff;
        }
    }
}
 ...
 * dinv is h2/A(i,i), e.g. h2/6.0 for 3D laplacian stencil
 * if store_residual is true, the residual will be stored into global field r */
__kernel void jacobi_stencil_iter_27point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict v_out, __global double *restrict f, __global double *restrict r,
    __global double *restrict stencil_values, const double h2inv, const double omega, const int m, const int n,
    const int o, const int ghosts, const int store_residual)
    ...
    if (j > ghosts - 1 && k > ghosts - 1 && j < n - ghosts && k < o - ghosts)
    {
        ...
        for (int i = ghosts; i < m - ghosts; i++)
        {
            ...
            double stencilsum = stencil_values[stencil_idx] * v_in_index +
                                ...
                                - stencil_values[stencil_idx + 7] * v_in[index - ioff - joff - koff] -
                                stencil_values[stencil_idx + 7] * v_in[index - ioff - joff + koff] -
                                stencil_values[stencil_idx + 7] * v_in[index - ioff + joff - koff] -
                                stencil_values[stencil_idx + 7] * v_in[index - ioff + joff + koff] -
                                stencil_values[stencil_idx + 7] * v_in[index + ioff - joff - koff] -
 ...
 * debug_out has the same dimensions as v_in.
 * ghosts is the amount of ghost cells of v_in in one direction */
__kernel void jacobi_stencil_stream_shmem_7point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict v_out, __global double *restrict f, __global double *restrict r,
    __global double *restrict stencil_values, __local double *vlocal, const double h2inv, const double omega,
    const int m, const int n, const int o, const int ghosts, const int iterations, const int store_residual)
{
    // extended block dimensions (field stored in shared memory has one more row and column)
    int blockDimExX = get_local_size(0) + 2;
    int blockDimExY = get_local_size(1) + 2;
    ...
    if (jreal < n && kreal < o)
    {
        ...
        for (int x = 1; x < iterations * 2 - 1; x++)
        {
            ...
            tidx = blockSizeEx + index_loc;
            index_f = index;
            stencil_idx = index_f << 2; // << 2 = * 4
            for (int t = 0; x - t >= t + 1; t++)
            {
                // calculate and store stencil
                stencilsum =
                    stencil_values[stencil_idx] * A[tidx_old] + stencil_values[stencil_idx + 1] * A[tidx_old - koff] +
                    stencil_values[stencil_idx + 1] * A[tidx_old + koff] +
                    stencil_values[stencil_idx + 2] * A[tidx_old - joff] +
                    stencil_values[stencil_idx + 2] * A[tidx_old + joff] +
                    stencil_values[stencil_idx + 3] * rm[tidx_old] + stencil_values[stencil_idx + 3] * rp[tidx_old];

                res = f[index_f] - stencilsum;
                rm[tidx] = A[tidx_old] + omega * (1.0 / stencil_values[stencil_idx]) * res;
                barrier(CLK_LOCAL_MEM_FENCE);
        ...
        for (int x = iterations * 2 - 1; x < m - 1; x++)
        {
            ...
            stencilsum =
                ...
                stencil_values[stencil_idx + 1] * A[tidx_old + koff] +
                stencil_values[stencil_idx + 2] * A[tidx_old - joff] +
                stencil_values[stencil_idx + 2] * A[tidx_old + joff] + stencil_values[stencil_idx + 3] * rm[tidx_old] +
                stencil_values[stencil_idx + 3] * rp[tidx_old];
            ...
            if (get_local_id(0) >= iterations - 1 && get_local_id(0) < get_local_size(0) - (iterations - 1) &&
                get_local_id(1) >= iterations - 1 && get_local_id(1) < get_local_size(1) - (iterations - 1))
                v_out[index_f] = A[tidx_old] + omega * (1.0 / stencil_values[stencil_idx]) * res;

            // if (jreal == ghosts && kreal == ghosts && x >= ghosts && x <= ghosts)
            // {
            //     printf("get_num_groups = %d,%d\n", (int)get_num_groups(0), (int)get_num_groups(1));
            //     printf("jreal,kreal = %d,%d; global_id = %d,%d; local_id = %d,%d; group_id = %d,%d\n", jreal, kreal,
            //     (int)get_global_id(0), (int)get_global_id(1), (int)get_local_id(0), (int)get_local_id(1),
            //     (int)get_group_id(0), (int)get_group_id(1)); printf("index_f = %d\n", index_f); printf("x = %d, res =
            //     %e, v_out = %e\n", x, res, v_out[index_f]); printf("stencilsum = %e\n", stencilsum);
            //     print_7point_local(A, rm, rp, tidx_old, ioff, joff, koff);
            // }
 ...
 * debug_out has the same dimensions as v_in.
 * ghosts is the amount of ghost cells of v_in in one direction */
__kernel void jacobi_stencil_stream_shmem_19point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict v_out, __global double *restrict f, __global double *restrict r,
    __global double *restrict stencil_values, __local double *vlocal, const double h2inv, const double omega,
    const int m, const int n, const int o, const int ghosts, const int iterations, const int store_residual)
{
    ...
    if (jreal < n && kreal < o)
    {
        ...
        for (int x = 1; x < iterations * 2 - 1; x++)
        {
            ...
            for (int t = 0; x - t >= t + 1; t++)
            {
                ...
                stencilsum =
                    ...
                    - stencil_values[stencil_idx + 4] * A[tidx_old - joff - koff] -
                    stencil_values[stencil_idx + 4] * A[tidx_old - joff + koff] -
                    stencil_values[stencil_idx + 4] * A[tidx_old + joff - koff] -
                    stencil_values[stencil_idx + 4] * A[tidx_old + joff + koff] -
                    stencil_values[stencil_idx + 6] * rm[tidx_old - koff] -
                    stencil_values[stencil_idx + 6] * rm[tidx_old + koff] -
                    stencil_values[stencil_idx + 6] * rp[tidx_old - koff] -
                    stencil_values[stencil_idx + 6] * rp[tidx_old + koff] -
                    stencil_values[stencil_idx + 5] * rm[tidx_old - joff] -
                    stencil_values[stencil_idx + 5] * rm[tidx_old + joff] -
                    stencil_values[stencil_idx + 5] * rp[tidx_old - joff] -
                    stencil_values[stencil_idx + 5] * rp[tidx_old + joff];
        ...
        for (int x = iterations * 2 - 1; x < m - 1; x++)
        {
            ...
            // calculate last time step and store in global memory
            stencilsum = stencil_values[stencil_idx] * A[tidx_old] +
                         stencil_values[stencil_idx + 1] * A[tidx_old - koff] +
                         stencil_values[stencil_idx + 1] * A[tidx_old + koff] +
                         stencil_values[stencil_idx + 2] * A[tidx_old - joff] +
                         stencil_values[stencil_idx + 2] * A[tidx_old + joff] +
            ...
            // if (jreal == 1 && kreal == 1 && x >= 0 && x <= 6)
            // {
            //     printf("x = %d\tidx = %d\tres = %e\tv_out = %e\n", x, index_f, res, A[tidx_old] + omega * res);
            //     print_19point_local(A, rm, rp, tidx_old, ioff, joff, 1);
            // }
 ...
 * debug_out has the same dimensions as v_in.
 * ghosts is the amount of ghost cells of v_in in one direction */
__kernel void jacobi_stencil_stream_shmem_27point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict v_out, __global double *restrict f, __global double *restrict r,
    __global double *restrict stencil_values, __local double *vlocal, const double h2inv, const double omega,
    const int m, const int n, const int o, const int ghosts, const int iterations, const int store_residual)
        ...
        for (int x = 1; x < iterations * 2 - 1; x++)
        {
            ...
            for (int t = 0; x - t >= t + 1; t++)
            {
                ...
                stencilsum =
                    ...
                    - stencil_values[stencil_idx + 7] * rm[tidx_old - joff - 1] -
                    stencil_values[stencil_idx + 7] * rm[tidx_old - joff + 1] -
                    stencil_values[stencil_idx + 7] * rm[tidx_old + joff - 1] -
                    stencil_values[stencil_idx + 7] * rm[tidx_old + joff + 1] -
                    stencil_values[stencil_idx + 7] * rp[tidx_old - joff - 1] -
                    stencil_values[stencil_idx + 7] * rp[tidx_old - joff + 1] -
                    stencil_values[stencil_idx + 7] * rp[tidx_old + joff - 1] -
                    stencil_values[stencil_idx + 7] * rp[tidx_old + joff + 1];
...
/* Calculates residual without dinv */
__kernel void residual_stencil_7point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict f, __global double *restrict r, __global double *restrict stencil_values,
    const double h2inv, const int m, const int n, const int o, const int ghosts)
{
    int i = get_global_id(0);
    int j = get_global_id(1);
    int k = get_global_id(2);
    ...
    if (i > ghosts - 1 && j > ghosts - 1 && k > ghosts - 1 && i < m - ghosts && j < n - ghosts && k < o - ghosts)
    {
        ...
        // A*v
        double stencilsum =
            stencil_values[stencil_idx] * v_in[index] + stencil_values[stencil_idx + 1] * v_in[index - koff] +
            stencil_values[stencil_idx + 1] * v_in[index + koff] +
            stencil_values[stencil_idx + 2] * v_in[index - joff] +
            stencil_values[stencil_idx + 2] * v_in[index + joff] +
            stencil_values[stencil_idx + 3] * v_in[index - ioff] + stencil_values[stencil_idx + 3] * v_in[index + ioff];
...
/* Calculates residual without dinv */
__kernel void residual_stencil_19point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict f, __global double *restrict r, __global double *restrict stencil_values,
    const double h2inv, const int m, const int n, const int o, const int ghosts)
{
...
/* Calculates residual without dinv */
__kernel void residual_stencil_27point(
    __global double *restrict v_in, // needed s.t. every work-item can read surrounding cell values
    __global double *restrict f, __global double *restrict r, __global double *restrict stencil_values,
    const double h2inv, const int m, const int n, const int o, const int ghosts)
{
