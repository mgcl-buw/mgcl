...
namespace mgcl_bench_residual_blockstencil
{
    ...
    enum class SEQ_LAYOUT
    {
        ...
        BS_GP_BLOCK_COEFFS_V_GP_FIRST  // [gpx][gpy][gpz][bi][bj][cx][cy][cz] for coeffs, [gpx][gpy][gpz][m] for v, f, r
    };
    ...
    // Layout for this one: [gpx][gpy][gpz][cx][cy][cz][bi][bj]
    class BlockstencilGPBlockCoeffs : public mgcl::Hypercube8d
    {
    public:
        BlockstencilGPBlockCoeffs(int m, int n, int o, int _width, int blocksize, int ghosts_m, int ghosts_n, int ghosts_o)
            : Hypercube8d(m, n, o, blocksize, blocksize, _width, _width, _width, ghosts_m, ghosts_n, ghosts_o, 0, 0, 0, 0, 0)
        {
            if (_width % 2 == 0 || _width < 1)
                error("Blockstencil is only defined for odd width >= 1!");
        ...
        BlockstencilGPBlockCoeffs(BlockstencilGPBlockCoeffs&) = delete;
        BlockstencilGPBlockCoeffs& operator=(const BlockstencilGPBlockCoeffs&) = delete;
        BlockstencilGPBlockCoeffs(BlockstencilGPBlockCoeffs&& o) : Hypercube8d(std::move(o)) {}
        BlockstencilGPBlockCoeffs& operator=(BlockstencilGPBlockCoeffs&& o)
        {
            Hypercube8d::operator=(std::move(o));
            return *this;
        }
        ...
        inline int getNgh() const { return dim2 + 2 * ghostsDim2; }
        inline int getOgh() const { return dim3 + 2 * ghostsDim3; }
        inline int getWidth() const { return dim6; }
        inline int getBlocksize() const { return dim4; }
    };

    using ResidualBSSeqCuboidVariant = std::variant<mgcl::CuboidBS, CuboidBSBlockFirst>;
    using ResidualBSSeqBlockstencilVariant = std::variant<
        mgcl::Blockstencil,
        BlockstencilCoeffsFirst,
        BlockstencilGPCoeffsBlock,
        BlockstencilGPBlockCoeffs>;
                    ...
                    for (int kv = kstart_v, kr = kstart_r, fk = kstart_f, ksv = kstart_sv; kv < kend_v; kv++, kr++, fk++, ksv++)
                    {
                        for (int bi = 0; bi < bs.getBlocksize(); bi++)
                        {
                            ...
                            for (int bj = 0; bj < bs.getBlocksize(); bj++)
                            {
                                ...
                                // clang-format off
                                stencilsum += bsraw[isv][jsv][ksv][bi][bj][1][1][1] * vraw[iv][jv][kv][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][1][0] * vraw[ iv ][ jv ][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][1][2] * vraw[ iv ][ jv ][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][0][1] * vraw[ iv ][jv-1][ kv ][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][2][1] * vraw[ iv ][jv+1][ kv ][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][1][1] * vraw[iv-1][ jv ][ kv ][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][1][1] * vraw[iv+1][ jv ][ kv ][bj]
                                    
                                    + bsraw[isv][jsv][ksv][bi][bj][1][0][0] * vraw[ iv ][jv-1][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][0][2] * vraw[ iv ][jv-1][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][2][0] * vraw[ iv ][jv+1][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][1][2][2] * vraw[ iv ][jv+1][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][1][0] * vraw[iv-1][ jv ][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][1][2] * vraw[iv-1][ jv ][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][1][0] * vraw[iv+1][ jv ][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][1][2] * vraw[iv+1][ jv ][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][0][1] * vraw[iv-1][jv-1][ kv ][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][2][1] * vraw[iv-1][jv+1][ kv ][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][0][1] * vraw[iv+1][jv-1][ kv ][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][2][1] * vraw[iv+1][jv+1][ kv ][bj]
                                    
                                    + bsraw[isv][jsv][ksv][bi][bj][0][0][0] * vraw[iv-1][jv-1][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][0][2] * vraw[iv-1][jv-1][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][2][0] * vraw[iv-1][jv+1][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][0][2][2] * vraw[iv-1][jv+1][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][0][0] * vraw[iv+1][jv-1][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][0][2] * vraw[iv+1][jv-1][kv+1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][2][0] * vraw[iv+1][jv+1][kv-1][bj]
                                    + bsraw[isv][jsv][ksv][bi][bj][2][2][2] * vraw[iv+1][jv+1][kv+1][bj];
                                // clang-format on
    ...
    TEST_CASE("seq_residualBlockstencilLayouts")
    {
        ...
        for (auto gr : gridsTBT)
        {
...

            {
                ...
                std::string name = std::string("residual_seq_blockstencil_gp_block_coeffs_")
                                       .append(std::to_string(m))
                                       .append("_")
                                       .append(std::to_string(n))
                                       .append("_")
