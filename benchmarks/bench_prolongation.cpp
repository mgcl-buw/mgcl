...
namespace mgcl_bench_prolongation
{
    ...
    int prolongate(mgcl::Problem& problem, mgcl::CuboidGpu& fine, mgcl::CuboidGpu& coarse, KernelVersion kernelVersion, std::vector<size_t> wgsizes)
    {
        ...
        if (kernelVersion == KernelVersion::THREE_D)
            ...
            kernelName = "prolongate_to_fine_4wi_per_gp";
        else if (kernelVersion == KernelVersion::THREE_D_2WI_PER_GP)
            kernelName = "prolongate_to_fine_2wi_per_gp";
    ...
    TEST_CASE("benchProlongation")
    {
        ...
        for (auto gr : gridsTBT)
        {
            ...
            if (false)
                for (auto wg : wg_sizes_1d)
                {
                    ...
                    bench.run(std::string(name).c_str(), [&] { //
                        prolongate(p, fine, coarse, KernelVersion::ONE_D_8WI_PER_GP, wg);
                        p.finish();
                    });
            ...
            for (auto wg : wg_sizes_3d)
            {
                ...
                bench.run(std::string(name).c_str(), [&] { //
                    prolongate(p, fine, coarse, KernelVersion::THREE_D_4WI_PER_GP, wg);
                    p.finish();
                });
                ...
                if (CLI_ARGS::checkResults)
                {
                    fine_3d_4wi_per_gp = std::make_unique<mgcl::Cuboid>(ml, nl, ol, ghosts, ghosts, ghosts);
                    lv0.getDVIn().read(p.getCommands(), fine_3d_4wi_per_gp.get(), true);
                }
            }
            ...
            for (auto wg : wg_sizes_3d)
            {
                ...
                bench.run(std::string(name).c_str(), [&] { //
                    prolongate(p, fine, coarse, KernelVersion::THREE_D_2WI_PER_GP, wg);
                    p.finish();
                });
                ...
                if (CLI_ARGS::checkResults)
                {
                    fine_3d_2wi_per_gp = std::make_unique<mgcl::Cuboid>(ml, nl, ol, ghosts, ghosts, ghosts);
                    lv0.getDVIn().read(p.getCommands(), fine_3d_2wi_per_gp.get(), true);
                }
            }
            ...
            if (CLI_ARGS::checkResults)
            {
                ...
                REQUIRE(fine_3d_prod->isEqual(*fine_3d_4wi_per_gp));
                REQUIRE(fine_3d_prod->isEqual(*fine_3d_2wi_per_gp));
