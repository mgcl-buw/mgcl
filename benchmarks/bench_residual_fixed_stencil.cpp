...
namespace mgcl_bench_residual_fixed_stencil
{
    enum class KernelVersion
    {
        GLOBAL_BUFFER,
        CONSTANTS,
        CONSTANTS_REORDERED_INDEX,
        PREPROCESSOR,
    ...
    double residual(ResidualArgs& args)
    {
        ...
        if (args.kernelVersion == KernelVersion::GLOBAL_BUFFER)
        {
            kernelName = "residual_27point_fixed_stencil_coeffs_as_global_buffer";
        }
        else if (args.kernelVersion == KernelVersion::CONSTANTS)
        {
            kernelName = "residual_27point_fixed_stencil_coeffs_as_constants";
        }
        else if (args.kernelVersion == KernelVersion::CONSTANTS_REORDERED_INDEX)
        {
            kernelName = "residual_27point_fixed_stencil_coeffs_as_constants_index_reordered";
        }
        else if (args.kernelVersion == KernelVersion::PREPROCESSOR)
        {
        ...
        else if (args.stencilType == mgcl::MGCL_FIXED && (args.kernelVersion == KernelVersion::CONSTANTS_REORDERED_INDEX || args.kernelVersion == KernelVersion::CONSTANTS))
        {
            // TODO avoid reading by keeping FixedStencil only on host, if it turns out to be fast
            auto fs = args.c_fixedStencil->read(args.commands, true);
            err = clSetKernelArg(kernel, pos, sizeof(cl_mem), &dVIn);
            err |= clSetKernelArg(kernel, ++pos, sizeof(cl_mem), &dF);
            err |= clSetKernelArg(kernel, ++pos, sizeof(cl_mem), &dR);
            err |= clSetKernelArg(kernel, ++pos, sizeof(int), &args.mgh);
        ...
        else if (args.stencilType == mgcl::MGCL_FIXED && args.kernelVersion == KernelVersion::GLOBAL_BUFFER)
        {
    ...
    TEST_CASE("residualFixedKernelVersions")
    {
        ...
        for (auto gr : gridsTBT)
        {
            ...
            auto fs_in = std::make_shared<mgcl::FixedStencil>(3);
            fs_in->fill1dIndex(false);
            mgcl::FixedStencilGpu c_fixedStencil(3, p.getContext(), p.getCommands(), p.getProgram());
            c_fixedStencil.fill(*fs_in, p.getCommands(), true);
            ...
            ResidualArgs args{
                ...
                .c_dF = c_dF,
                .c_dR = c_dR,
                .c_stencilValues = nullptr,
                .c_fixedStencil = &c_fixedStencil,
                .pd = p.getProfilingData(),
                .moff = 0,
                .noff = 0,
                .ooff = 0,
                .kernelVersion = KernelVersion::GLOBAL_BUFFER,
            };
            ...
            std::unique_ptr<mgcl::Cuboid> r_out_constants_reordered_index = nullptr;
            ...
            {
                args.kernelVersion = KernelVersion::GLOBAL_BUFFER;
                std::string name = std::string("residual_fixed_stencil_global_buffer_")
            ...
            {
                args.kernelVersion = KernelVersion::CONSTANTS;
                std::string name = std::string("residual_fixed_stencil_constants_")
                                       .append(std::to_string(m))
                                       .append("_")
                                       .append(std::to_string(n))
                                       .append("_")
            ...
            {
                args.kernelVersion = KernelVersion::CONSTANTS_REORDERED_INDEX;
                std::string name = std::string("residual_fixed_stencil_constants_reordered_index_")
                                       .append(std::to_string(m))
                                       .append("_")
                                       .append(std::to_string(n))
                                       .append("_")
                ...
                if (CLI_ARGS::checkResults)
                {
                    r_out_constants_reordered_index = std::make_unique<mgcl::Cuboid>(m, n, o, ghosts, ghosts, ghosts);
                    args.c_dR.read(args.commands, r_out_constants_reordered_index.get(), true);
                }
            }
            ...
            if (CLI_ARGS::checkResults)
            {
                ...
                REQUIRE(r_out_global_buffer->isEqual(*r_out_constants_reordered_index));
