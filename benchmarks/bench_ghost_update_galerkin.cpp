...
// This benchmark shall answer the question of how big the time portion is, that the ghost update of
// the varying stencil needs, compared to the actual calculation of Galerkin, when using MPI and OCL.
// Timings will be collected per node and printed by rank at the end.
// Must be called with mulitple MPI processes.
// There are 2 tests done:
// - galerkin only
// - ghost update only
// Run with e.g.: mpiexec -n 4 bench_ghost_update_mpi_ocl_galerkin
TEST_CASE("bench_ghost_update_mpi_ocl_galerkin")
{
    using std::min;
    ...
    for (auto gr : gridsTBT)
    {
        ...
        for (int ghosts = 1; ghosts <= maxGhosts; ghosts++)
        {
            ...
            auto sv_ptr = p->getLevelAt(0).getStencilValuesGpu();
            auto mpiData = p->getLevelAt(0).getMpiDataPtr();
            ...
            // Galerkin
            {
                std::string name = std::string("galerkin_")
                ...
                bench.run(std::string(name).c_str(), [&] { //
                    ...
                    mgcl::MultigridEngine::galerkinOptimized(sv, 1, m >> 1, n >> 1, o >> 1, p->getProgram(),
                                                             p->getCommands(), p->getContext(), nullptr, nullptr);
            ...
            // Ghost Update
            {
                std::string name = std::string("ghost_update_")
                                       .append(std::to_string(m))
                                       .append("_")
                                       .append(std::to_string(n))
                                       .append("_")
