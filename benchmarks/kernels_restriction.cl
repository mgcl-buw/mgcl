 ...
 * uses a 1d kernel, which parallelizes all three loop in x,y and z directions.
 * global size must be of ghosted grid.
 * Uses Blockjacobi, i.e. Problem::smootherType == MGCL_JACOBI_BLOCK.
 * mgh, ngh and ogh must be dimensions of local ghosted grid, too.
 * svmgh, svngh and svogh are ghosted grid sizes of stencilValues (might differ from mgh,ngh,ogh when using MPI).
 * if store_residual is true, the residual will be stored into global field r.
 * stencilValues is a VaryingStencilGpu having width 3 (i.e. a 6d array).
 ...
 * uses a 1d kernel, which parallelizes all three loop in x,y and z directions.
 * global size must be of ghosted grid.
 * Uses scalar Jacobi, i.e. Problem::smootherType == MGCL_JACOBI_SCALAR.
 * mgh, ngh and ogh must be dimensions of local ghosted grid, too.
 * svmgh, svngh and svogh are ghosted grid sizes of stencilValues (might differ from mgh,ngh,ogh when using MPI).
 * if store_residual is true, the residual will be stored into global field r.
 * stencilValues is a VaryingStencilGpu having width 3 (i.e. a 6d array).
 ...
 * Needs to get called with m*n*o work-items (real grid).
    ...
    const int ngh_vals_coarse, const int ogh_vals_coarse)
{
    ...
    if (i < mgh && j < ngh && k < ogh)
    {
        ...
        const int nf = (ngh - g2) * 2 + g2, of = (ogh - g2) * 2 + g2;
...
__kernel void restrict_to_coarse_2wi_per_gp(
    __global double* restrict fine,
    __global double* restrict coarse,
    __local double* partials, // size = (wg-size / 2) * 1
    const int mgh, const int ngh, const int ogh, const int ghosts,
    const int ngh_vals_coarse, const int ogh_vals_coarse)
{
    int gridPointsPerBlock = get_local_size(0) / 2;
    int i = get_global_id(2);
    int j = get_global_id(1);
    ...
    if (i < mgh && j < ngh && k < ogh)
    {
        ...
        if (get_local_id(0) < gridPointsPerBlock)
        {
            sum =
                0.125 * fine[i2 * nf * of + j2 * of + k2] // self
                // direct neighbours
                + 0.0625 * fine[(i2 - 1) * nf * of + j2 * of + k2] +
                0.0625 * fine[(i2 + 1) * nf * of + j2 * of + k2] +
                0.0625 * fine[i2 * nf * of + (j2 - 1) * of + k2] +
                0.0625 * fine[i2 * nf * of + (j2 + 1) * of + k2] +
                0.0625 * fine[i2 * nf * of + j2 * of + k2 - 1] +
                0.0625 * fine[i2 * nf * of + j2 * of + k2 + 1]
                // edge midpoints xy-plane
        ...
        if (get_local_id(0) < gridPointsPerBlock)
        {
            sum += partials[get_local_id(0) + gridPointsPerBlock];
...
__kernel void restrict_to_coarse_4wi_per_gp(
    __global double* restrict fine,
    __global double* restrict coarse,
    ...
    const int ngh_vals_coarse, const int ogh_vals_coarse)
{
    int gridPointsPerBlock = get_local_size(0) / 4;
    int i = get_global_id(2);
    int j = get_global_id(1);
    int k = get_global_id(0) % gridPointsPerBlock + (get_global_id(0) / get_local_size(0)) * gridPointsPerBlock;
    int g2 = 2 * ghosts;
    ...
    if (i < mgh && j < ngh && k < ogh)
    {
        ...
        {
            sum += partials[get_local_id(0) + gridPointsPerBlock];
            sum += partials[get_local_id(0) + gridPointsPerBlock * 2];
            sum += partials[get_local_id(0) + gridPointsPerBlock * 3];
            // if (i == ghosts && j == ghosts && k == ghosts)
            //     printf("get_local_id(0) %d reading from %d\n", get_local_id(0), get_local_id(0));
