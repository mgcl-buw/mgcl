...
TEST_CASE("benchmark var*fix stencils optimizations", "[console][VarFixKernelOptimizations]")
{
    ...
    // reordered for loops + preserved loop bounds
    // no significant benefit on Laptop gpu
    {
        int err;

        // Create the compute kernel from the program
        cl_kernel kernel = clCreateKernel(oclw.program, "mult_stencils_var_fix_reordered_loop_bounds_preserved", &err);
        mgcl::mgclCheckError(err, "clCreateKernel");
        ...
        // enqueue multiplication kernel
        std::string name = std::string("reordered loop bounds preserved var*fix, N = ").append(std::to_string(N));
        bench.run(std::string(name).c_str(), [&]
                  { 
            err = clEnqueueNDRangeKernel(oclw.commands, kernel, 3, NULL, global, local, 0, NULL, NULL);
            mgcl::mgclCheckError(err, "Enqueueing stencil multiplication kernel");
    ...
    // reordered for loops, loops over c parallelized, too
    // huge benefit
    {
        int err;

        // Create the compute kernel from the program
        cl_kernel kernel = clCreateKernel(oclw.program, "mult_stencils_var_fix_reordered_parallel_c", &err);
        mgcl::mgclCheckError(err, "clCreateKernel");
        ...
        // enqueue multiplication kernel
        std::string name = std::string("reordered parallel c var*fix, N = ").append(std::to_string(N));
        bench.run(std::string(name).c_str(), [&]
                  { 
            err = clEnqueueNDRangeKernel(oclw.commands, kernel, 3, NULL, global, local, 0, NULL, NULL);
            mgcl::mgclCheckError(err, "Enqueueing stencil multiplication kernel");
...
TEST_CASE("benchmark fix*var stencils optimizations", "[console][FixVarKernelOptimizations]")
{
    ...
    // reordered for loops + parallel c loop
    // huge benefit
    {
        int err;

        // Create the compute kernel from the program
        cl_kernel kernel = clCreateKernel(oclw.program, "mult_stencils_fix_var_reordered_parallel_c", &err);
        mgcl::mgclCheckError(err, "clCreateKernel");
