...
namespace mgcl_bench_restriction
{
    ...
    int restrict(mgcl::Problem& problem, mgcl::CuboidGpu& fine, mgcl::CuboidGpu& coarse, KernelVersion kernelVersion, std::vector<size_t> wgsizes)
    {
        int err;

        bool is3d = (kernelVersion == KernelVersion::THREE_D ||
                     kernelVersion == KernelVersion::THREE_D_8WI_PER_GP ||
                     kernelVersion == KernelVersion::THREE_D_4WI_PER_GP ||
                     kernelVersion == KernelVersion::THREE_D_2WI_PER_GP);

        // Create the compute kernel from the program
        std::string kernelName;
        if (kernelVersion == KernelVersion::THREE_D)
            kernelName = "restrict_to_coarse";
        // else if (kernelVersion == KernelVersion::THREE_D_8WI_PER_GP)
        //     kernelName = "prolongate_to_fine_8wi_per_gp";
        else if (kernelVersion == KernelVersion::THREE_D_4WI_PER_GP)
            kernelName = "restrict_to_coarse_4wi_per_gp";
        else if (kernelVersion == KernelVersion::THREE_D_2WI_PER_GP)
            kernelName = "restrict_to_coarse_2wi_per_gp";
        ...
        int coarse_m = coarse.getM();
        int coarse_n = coarse.getN();
        ...
        if (kernelVersion == KernelVersion::THREE_D_2WI_PER_GP)
        {
            // size_t locsize = (wgsizes[0] * wgsizes[1] * wgsizes[2]) / 2;
            size_t locsize = wgsizes[0];
            err |= clSetKernelArg(kernel, ++pos, sizeof(double) * locsize, NULL);
        }

        if (kernelVersion == KernelVersion::THREE_D_4WI_PER_GP)
        {
            // size_t locsize = ((wgsizes[0] * wgsizes[1] * wgsizes[2]) / 4) * 3;
            size_t locsize = wgsizes[0];
            err |= clSetKernelArg(kernel, ++pos, sizeof(double) * locsize, NULL);
        }
        ...
        //     kernelVersion == KernelVersion::THREE_D_4WI_PER_GP ||
        //     kernelVersion == KernelVersion::THREE_D_2WI_PER_GP ||
        //     kernelVersion == KernelVersion::ONE_D_8WI_PER_GP ||
        ...
        //     err |= clSetKernelArg(kernel, ++pos, sizeof(int), &coarse_ngh);
        //     err |= clSetKernelArg(kernel, ++pos, sizeof(int), &coarse_ogh);
        //     err |= clSetKernelArg(kernel, ++pos, sizeof(int), &coarse_m);
        //     err |= clSetKernelArg(kernel, ++pos, sizeof(int), &coarse_n);
        //     err |= clSetKernelArg(kernel, ++pos, sizeof(int), &coarse_o);
        // }

        mgcl::mgclCheckError(err, "Setting kernel arguments");

        size_t global[3] = {static_cast<size_t>(coarse_o), static_cast<size_t>(coarse_n), static_cast<size_t>(coarse_m)};
        ...
        if (kernelVersion == KernelVersion::THREE_D_4WI_PER_GP)
        {
            ...
            global[0] = static_cast<size_t>(coarse.getO() * 4);
        ...
        {
            // launch for real points only
            global[0] = static_cast<size_t>(coarse.getO() * 2);
            global[1] = static_cast<size_t>(coarse.getN());
            global[2] = static_cast<size_t>(coarse.getM());
        }
    ...
    TEST_CASE("benchRestriction")
    {
        ...
        for (auto gr : gridsTBT)
        {
            ...
            p.setSilent(true);
            p.setKernelFile("kernels_restriction.cl");
            if (CLI_ARGS::useBinaryFile)
            {
                p.setBinaryFile("kernels_restriction.bin");
            }
            p.setUseOpencl(true);
            p.setGhosts(ghosts);
            ...
            std::unique_ptr<mgcl::Cuboid> coarse_3d_prod = nullptr;
            std::unique_ptr<mgcl::Cuboid> coarse_3d_1wi_per_gp = nullptr;
            // std::unique_ptr<mgcl::Cuboid> fine_3d_8wi_per_gp = nullptr;
            std::unique_ptr<mgcl::Cuboid> fine_3d_4wi_per_gp = nullptr;
            std::unique_ptr<mgcl::Cuboid> fine_3d_2wi_per_gp = nullptr;
            ...
            if (CLI_ARGS::checkResults)
            {
                ...
                coarse_3d_prod = coarse.read(p.getCommands(), nullptr, false);
                auto h_fine = fine.read(p.getCommands(), nullptr, true);

                mgcl::MultigridEngine::restrictSeq(lv0, lv1, *h_fine, *coarse_3d_prod);
            }

            // std::vector<std::vector<size_t>> wg_sizes_1d = {{8, 1, 1}, {32, 1, 1}, {64, 1, 1}, {128, 1, 1}, {256, 1, 1}};
            // if (false)
            ...
            //         std::string name = std::string("prolongation_8wi_per_gp_1d_")
            ...
            //             restrict(p, fine, coarse, KernelVersion::ONE_D_8WI_PER_GP, wg);
            //             p.finish();
            //         });
            ...
            //         bench.run(std::string(name).c_str(), [&] { //
            //             restrict(p, fine, coarse, KernelVersion::ONE_D_4WI_PER_GP, wg);
            //             p.finish();
            //         });
            ...
            {
                {
                    ...
                    bench.run(std::string(name).c_str(), [&] { //
                        restrict(p, fine, coarse, KernelVersion::THREE_D, wg);
                        p.finish();
                    });
            ...
            //     bench.run(std::string(name).c_str(), [&] { //
            //         restrict(p, fine, coarse, KernelVersion::THREE_D_8WI_PER_GP, wg);
            //         p.finish();
            //     });
            ...
            for (auto wg : wg_sizes_3d)
            {
                ...
                bench.run(std::string(name).c_str(), [&] { //
                    restrict(p, fine, coarse, KernelVersion::THREE_D_4WI_PER_GP, wg);
                    p.finish();
                });
            ...
            for (auto wg : wg_sizes_3d)
            {
                ...
                bench.run(std::string(name).c_str(), [&] { //
                    restrict(p, fine, coarse, KernelVersion::THREE_D_2WI_PER_GP, wg);
                    p.finish();
                });
            ...
            if (CLI_ARGS::checkResults)
            {
                ...
                // coarse_3d_prod->dumpToFile("coarse_3d_prod.txt");
                // coarse_3d_1wi_per_gp->dumpToFile("coarse_3d_1wi_per_gp.txt");

                REQUIRE(coarse_3d_prod->isEqual(*coarse_3d_1wi_per_gp));
                // REQUIRE(coarse_3d_prod->isEqual(*fine_3d_8wi_per_gp));
                REQUIRE(coarse_3d_prod->isEqual(*fine_3d_4wi_per_gp));
                REQUIRE(coarse_3d_prod->isEqual(*fine_3d_2wi_per_gp));
                // if (fine_1d_8wi_per_gp)
                // REQUIRE(coarse_3d_prod->isEqual(*fine_1d_8wi_per_gp));
                // if (fine_1d_4wi_per_gp)
                // REQUIRE(coarse_3d_prod->isEqual(*fine_1d_4wi_per_gp));
            }
