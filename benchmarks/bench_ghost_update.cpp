...
TEST_CASE("bench_ghostupdate_mpi_ocl_steps")
{
    ...
    for (auto gr : gridsTBT)
    {
...

        {
            ...
            bench.run(std::string(name).c_str(), [&] { //
                auto sbuf_ptr = c_d.extractBorderPlanes(p.getCommands(), p.getProgram(), &d_planesbuf, nullptr);
                p.getOpenCLHelper().finish();
            });
...

        {
            ...
            bench.run(std::string(name).c_str(), [&] { //
                c_d.extractBorderPlanes(p.getCommands(), p.getProgram(), &d_planesbuf, &h_planesbuf);
                p.getOpenCLHelper().finish();
            });
...

        {
            ...
            bench.run(std::string(name).c_str(), [&] { //
                c_d.pasteGhostsFromBorderPlanes(p.getContext(), p.getCommands(), p.getProgram(), nullptr, &rbuf);
                p.getOpenCLHelper().finish();
            });
...

        {
            ...
            bench.run(std::string(name).c_str(), [&] { //
                c_d.pasteGhostsFromBorderPlanes(p.getContext(), p.getCommands(), p.getProgram(), &d_planesbuf, nullptr);
                p.getOpenCLHelper().finish();
            });
