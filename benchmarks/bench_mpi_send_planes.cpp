...
TEST_CASE("bench_sendBorderPlanes")
{
    ...
    mgcl::MPILevelData mpiLevelData(mpi_comm, 1);
    {
        /* Calculating neighbours */
        int ret;
        ret = MPI_Cart_shift(mpiLevelData.comm, 2, 1, &mpiLevelData.left[0], &mpiLevelData.right[0]);
        mgcl::mpi_util::mgclCheckMpiError(mpiLevelData.comm, ret, "MPI_Cart_shift x-direction");
        ret = MPI_Cart_shift(mpiLevelData.comm, 1, 1, &mpiLevelData.down[0], &mpiLevelData.up[0]);
        mgcl::mpi_util::mgclCheckMpiError(mpiLevelData.comm, ret, "MPI_Cart_shift y-direction");
        ret = MPI_Cart_shift(mpiLevelData.comm, 0, 1, &mpiLevelData.front[0], &mpiLevelData.back[0]);
                      ...
                      std::vector<double>& sbuf, mgcl::MPILevelData& mpiData)
{
    ...
    // Send front planes to the back
    err = MPI_Sendrecv_replace(static_cast<void*>(sbuf.data()), yzgh * stencilSize, MPI_DOUBLE, mpiData.back[0], 0,
                               mpiData.front[0], 0,
                               mpiData.comm, MPI_STATUS_IGNORE);
    mgcl::mpi_util::mgclCheckMpiError(mpiData.comm, err, "MPI_Sendrecv");

    // Send back planes to the front
    err = MPI_Sendrecv_replace(static_cast<void*>(&(sbuf[base_yz_back])), yzgh * stencilSize, MPI_DOUBLE, mpiData.front[0], 0,
                               mpiData.back[0], 0,
                               mpiData.comm, MPI_STATUS_IGNORE);
    mgcl::mpi_util::mgclCheckMpiError(mpiData.comm, err, "MPI_Sendrecv");
    ...
    // Send top planes to the bottom
    err = MPI_Sendrecv_replace(static_cast<void*>(&(sbuf[base_xz_top])), xzgh * stencilSize, MPI_DOUBLE, mpiData.down[0], 0,
                               mpiData.up[0], 0,
                               mpiData.comm, MPI_STATUS_IGNORE);
    mgcl::mpi_util::mgclCheckMpiError(mpiData.comm, err, "MPI_Sendrecv");

    // Send bottom planes to the top
    err = MPI_Sendrecv_replace(static_cast<void*>(&(sbuf[base_xz_bottom])), xzgh * stencilSize, MPI_DOUBLE, mpiData.up[0], 0,
                               mpiData.down[0], 0,
                               mpiData.comm, MPI_STATUS_IGNORE);
    mgcl::mpi_util::mgclCheckMpiError(mpiData.comm, err, "MPI_Sendrecv");
    ...
    // Send left planes to the right
    err = MPI_Sendrecv_replace(static_cast<void*>(&(sbuf[base_xy_left])), xygh * stencilSize, MPI_DOUBLE, mpiData.right[0], 0,
                               mpiData.left[0], 0,
                               mpiData.comm, MPI_STATUS_IGNORE);
    mgcl::mpi_util::mgclCheckMpiError(mpiData.comm, err, "MPI_Sendrecv");

    // Send right planes to the left
    err = MPI_Sendrecv_replace(static_cast<void*>(&(sbuf[base_xy_right])), xygh * stencilSize, MPI_DOUBLE, mpiData.left[0], 0,
                               mpiData.right[0], 0,
                               mpiData.comm, MPI_STATUS_IGNORE);
    mgcl::mpi_util::mgclCheckMpiError(mpiData.comm, err, "MPI_Sendrecv");
}