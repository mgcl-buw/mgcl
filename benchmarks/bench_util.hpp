...
namespace bench_util
{
    ...
    inline void printCsvFormat(std::vector<Result> results)
    {
        // print min times
        std::stringstream ss;
        ss << std::endl;
    ...
    inline void printCsvFormat(std::vector<ResultJacobiMpi> results, MPI_Comm mpi_comm, int mpi_rank)
    {
        ...
        if (mpi_rank == 0)
        {
            ...
            ss << "***DATASTART***" << std::endl;
            // ss << "gpus;spi;iters;name;mloc;nloc;oloc;mglob;nglob;oglob;minTimeInMs;medianTimeInMs;avgTimeInMs;err%" << std::endl;
            ss << "name;gpus;spi;iters;mloc;nloc;oloc;mglob;nglob;oglob;minTimeInMs;medianTimeInMs;avgTimeInMs;err%" << std::endl;
            for (auto r : results)
            {
                ss << r.name << ";" << r.gpus << ";" << r.spi << ";" << r.iters << ";"
                   /* << r.name << ";" */
    ...
    inline void printCsvFormat(std::vector<ResultDataTransferMpi> results, MPI_Comm mpi_comm, int mpi_rank)
    {
        // print min times
        MPI_Barrier(mpi_comm);
        if (mpi_rank == 0)
        {
            ...
            ss << "***DATASTART***" << std::endl;
            // ss << "gpus;spi;iters;name;mloc;nloc;oloc;mglob;nglob;oglob;minTimeInMs;medianTimeInMs;avgTimeInMs;err%" << std::endl;
            ss << "gpus;name;elements;minTimeInMs;medianTimeInMs;avgTimeInMs;err%" << std::endl;
            for (auto r : results)
            {
                ss << r.gpus << ";" << r.name << ";" << r.elements
                   << ";" << std::setprecision(17) << r.minTime
                   << ";" << std::setprecision(17) << r.medianTime
                   << ";" << std::setprecision(17) << r.avgTime
                   << ";" << std::setprecision(4) << r.medianAbsolutePercentError << "%"
