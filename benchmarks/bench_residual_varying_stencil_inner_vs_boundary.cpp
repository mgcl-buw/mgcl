/**
 * @date 26.06.2025
 * This file contains code for benchmarking the residual kernel applied to only inner grid points vs. only boundary grid points.
 * This is needed when one wants to overlap Jacobi and ghost update.
 *
...
namespace mgcl_bench_residual_varying_inner_vs_boundary
{
    ...
    double residual(ResidualArgs& args)
    {
        ...
        if (args.kernelVersion == KernelVersion::INNER)
        {
            kernelName = "residual_27point_varying_stencil_inner";
        }
        else if (args.kernelVersion == KernelVersion::BOUNDARY)
        {
    ...
    TEST_CASE("residualVaryingKernelInnerVsBoundary")
    {
        using std::min;
        ...
        for (auto gr : gridsTBT)
        {
            ...
            ResidualArgs args{
                ...
                .noff = 0,
                .ooff = 0,
                .kernelVersion = KernelVersion::INNER,
            };
            ...
            {
                args.kernelVersion = KernelVersion::INNER;
                std::string name = std::string("residual_varying_stencil_inner_")
                ...
                if (CLI_ARGS::checkResults)
                {
                    r_out_inner = std::make_unique<mgcl::Cuboid>(m, n, o, ghosts, ghosts, ghosts);
                    args.c_dR.read(args.commands, r_out_inner.get(), true);
                }
            }

            {
                args.kernelVersion = KernelVersion::BOUNDARY;
                std::string name = std::string("residual_varying_stencil_boundary_")
                                       .append(std::to_string(m))
                                       .append("_")
                                       .append(std::to_string(n))
                                       .append("_")
                ...
                if (CLI_ARGS::checkResults)
                {
                    r_out_boundary = std::make_unique<mgcl::Cuboid>(m, n, o, ghosts, ghosts, ghosts);
                    args.c_dR.read(args.commands, r_out_boundary.get(), true);
                }
            }
            ...
            // if (CLI_ARGS::checkResults)
            {
                ...
                std::string name = std::string("residual_varying_stencil_default_")
                                       .append(std::to_string(m))
                                       .append("_")
                                       .append(std::to_string(n))
                                       .append("_")
                ...
                if (CLI_ARGS::checkResults)
                {
                    r_out_default = std::make_unique<mgcl::Cuboid>(m, n, o, ghosts, ghosts, ghosts);
                    args.c_dR.read(args.commands, r_out_default.get(), true);
                }
            }
            ...
            if (CLI_ARGS::checkResults)
            {
                for (int i = ghosts; i < m + ghosts; i++)
                    for (int j = ghosts; j < n + ghosts; j++)
                        ...
                        {
                            if (i > ghosts && i < m + ghosts - 1 &&
                                j > ghosts && j < n + ghosts - 1 &&
                                k > ghosts && k < o + ghosts - 1)
                            {
                                REQUIRE((*r_out_default)[i][j][k] == (*r_out_inner)[i][j][k]);
                            }
                            else if (i == ghosts || i == m + ghosts - 1 ||
                                     j == ghosts || j == n + ghosts - 1 ||
                                     k == ghosts || k == o + ghosts - 1)
                            {
