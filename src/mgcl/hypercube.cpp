...
namespace mgcl
{
    ...
    /**
     * @brief Construct a new Hypercube5d object having ghosts = 0 and optionally a value which defaults to 0.
     *
     * @param dim1_ Dimension of real grid.
     * @param dim2_ Dimension of real grid.
     * @param dim3_ Dimension of real grid.
     ...
     * @param value Initial value, defaults to 0.
     */
    Hypercube5d::Hypercube5d(int dim1_, int dim2_, int dim3_, int dim4_, int dim5_, double value)
        : Hypercube5d(dim1_, dim2_, dim3_, dim4_, dim5_, 0, 0, 0, 0, 0, value) {}

    /**
     * @brief Construct a new ghosted Hypercube5d object, optionally having a value which defaults to 0.
     *
     * @param dim1_ Dimension of real grid.
     * @param dim2_ Dimension of real grid.
     * @param dim3_ Dimension of real grid.
     ...
     * @param value Initial value, defaults to 0.
     */
    Hypercube5d::Hypercube5d(int dim1_, int dim2_, int dim3_, int dim4_, int dim5_,
                             int ghostsDim1_, int ghostsDim2_, int ghostsDim3_, int ghostsDim4_, int ghostsDim5_, double value)
        : dim1(dim1_),
          dim2(dim2_),
          dim3(dim3_),
          dim4(dim4_),
          ...
          size(dim1gh * dim2gh * dim3gh * dim4gh * dim5gh)
    {
        field_1d.resize(size);
        for (int i = 0; i < field_1d.size(); i++)
            field_1d[i] = value;

        field_5d = new double****[dim1gh];
        int size4gh = dim5gh;
        ...
        {
            field_5d[d1] = new double***[dim2gh];
            for (int d2 = 0; d2 < dim2gh; d2++)
            {
                field_5d[d1][d2] = new double**[dim3gh];
                for (int d3 = 0; d3 < dim3gh; d3++)
                {
                    field_5d[d1][d2][d3] = new double*[dim4gh];
                    for (int d4 = 0; d4 < dim4gh; d4++)
                    {
                        field_5d[d1][d2][d3][d4] = &field_1d[d1 * size1gh + d2 * size2gh + d3 * size3gh + d4 * size4gh];
                    }
                }
            }
        }
    ...
    Hypercube5d::Hypercube5d(Hypercube5d&& o)
        : dim1(o.dim1),
          dim2(o.dim2),
          dim3(o.dim3),
          dim4(o.dim4),
    ...
    Hypercube5d& Hypercube5d::operator=(Hypercube5d&& o)
    {
        dim1 = o.dim1;
        dim2 = o.dim2;
        dim3 = o.dim3;
    ...
    /**
     * @brief Destroy the Hypercube5d object, freeing memory.
     */
        ...
        if (field_5d)
        {
            for (int d1 = 0; d1 < dim1gh; d1++)
            {
                for (int d2 = 0; d2 < dim2gh; d2++)
                {
                    for (int d3 = 0; d3 < dim3gh; d3++)
                    {
                        delete[] field_5d[d1][d2][d3];
                    }
                    delete[] field_5d[d1][d2];
                }
                delete[] field_5d[d1];
            }
            delete[] field_5d;
    ...
    /**
     * @brief Fills Hypercube5d with random values between low and high, which default to 0 and 1.
     *
     */
    void Hypercube5d::fillRandom(double low, double high)
    {
        std::random_device dev;
        std::mt19937 rng(dev());
        std::uniform_real_distribution<double> dist(low, high);
    ...
    void Hypercube5d::fillRandomInt(int low, int high, bool realCellsOnly)
    {
                ...
                for (int d2 = ghostsDim2; d2 < dim2 + ghostsDim2; d2++)
                    for (int d3 = ghostsDim3; d3 < dim3 + ghostsDim3; d3++)
                        for (int d4 = ghostsDim4; d4 < dim4 + ghostsDim4; d4++)
                            ...
                            {
                                field_5d[d1][d2][d3][d4][d5] = dist(rng);
                            }
        }
        else
        {
    ...
    /**
     * @brief Fills Hypercube5d with given value
     *
     * @param value Value to fill
     * @param realCellsOnly If true, only real cells will be set to value. Defaults to false.
     */
    void Hypercube5d::fill(double value, bool realCellsOnly)
    {
        if (realCellsOnly)
        {
            for (int d1 = ghostsDim1; d1 < dim1 + ghostsDim1; d1++)
    ...
    /**
     * @brief Fills this Hypercube5d with the 1d index of the corresponding cell, including ghost cells.
     *
     * @param realCellsOnly If true, only real cells get filled.
     */
    void Hypercube5d::fill1dIndex(bool realCellsOnly)
    {
        if (realCellsOnly)
            for (int d1 = ghostsDim1; d1 < dim1 + ghostsDim1; d1++)
                for (int d2 = ghostsDim2; d2 < dim2 + ghostsDim2; d2++)
                    for (int d3 = ghostsDim3; d3 < dim3 + ghostsDim3; d3++)
                        for (int d4 = ghostsDim4; d4 < dim4 + ghostsDim4; d4++)
                            ...
                            {
                                (*this)[d1][d2][d3][d4][d5] =
                                    d1 * dim2gh * dim3gh * dim4gh * dim5gh +
                                    d2 * dim3gh * dim4gh * dim5gh +
                                    d3 * dim4gh * dim5gh +
                                    d4 * dim5gh +
                                    d5;
                            }
    ...
    /**
     * @brief Returns true if real cells contents of this Hypercube5d is equal to the one of another Hypercube5d c
     * within a given tolerance tol, respecting ghost cell amount. Dimensions of real cell amount of this Hypercube5d
     *  and c must be equal (without ghost cells).
     *
     * @param c Other Hypercube5d
     * @param tol tolerance that is used for checking equality. Defaults to 1e-7.
     * @param printDiffs If true, differences will be printed to standard output. Defaults to true.
     * @return true Cuboids equal.
     * @return false Cuboids not equal.
     * @throws invalid_argument When dimensions of Cuboids don't match.
     */
    bool Hypercube5d::isEqual(Hypercube5d& c, double tol)
    {
        if (dim1 != c.getDim1() ||
            dim2 != c.getDim2() ||
            dim3 != c.getDim3() ||
            ...
            dim5 != c.getDim5())
        {
            error(std::invalid_argument("Cannot check equality for Hypercube5d. Dimensions differ."));
        }
            ...
            for (int d2 = 0; d2 < dim2; d2++)
                for (int d3 = 0; d3 < dim3; d3++)
                    for (int d4 = 0; d4 < dim4; d4++)
                        ...
                        {
                            diff = fabs(field_5d[d1 + ghostsDim1][d2 + ghostsDim2][d3 + ghostsDim3][d4 + ghostsDim4][d5 + ghostsDim5] -
                                        c[d1 + c.getGhostsDim1()][d2 + c.getGhostsDim2()][d3 + c.getGhostsDim3()][d4 + c.getGhostsDim4()][d5 + c.getGhostsDim5()]);
                            if (diff > tol)
                            {
                                return false;
                            }
     ...
     * @throws invalid_argument When dimensions of Cuboids don't match.
     */
    bool Hypercube5d::isEqualIncGhosts(Hypercube5d& c, double tol)
    {
            ...
            for (int d2 = 0; d2 < dim2gh; d2++)
                for (int d3 = 0; d3 < dim3gh; d3++)
                    for (int d4 = 0; d4 < dim4gh; d4++)
                        for (int d5 = 0; d5 < dim5gh; d5++)
                        {
                            diff = fabs(field_5d[d1][d2][d3][d4][d5] -
                                        c[d1][d2][d3][d4][d5]);
                            if (diff > tol)
                            {
                                return false;
     ...
     * @throws runtime_error When file could not be opened.
     */
    void Hypercube5d::dumpToFile(std::string path, bool realCellsOnly)
    {
        std::ofstream myfile;
        myfile.open(path, std::ios::out | std::ios::trunc);
                    ...
                    for (int d2 = ghostsDim2; d2 < dim2 + ghostsDim2; d2++)
                        for (int d3 = ghostsDim3; d3 < dim3 + ghostsDim3; d3++)
                            for (int d4 = ghostsDim4; d4 < dim4 + ghostsDim4; d4++)
                                for (int d5 = ghostsDim5; d5 < dim5 + ghostsDim5; d5++)
                                {
                                    myfile << d1 << "\t" << d2 << "\t" << d3 << "\t" << d4 << "\t" << d5 << "\t"
                                           << std::scientific << std::setprecision(17) << field_5d[d1][d2][d3][d4][d5] << std::endl;
                                }
            }
            else
            {
    ...
    void Hypercube5d::dumpToFileMatlab(std::string path, std::string varname, bool realCellsOnly)
    {
        std::ofstream myfile;
        myfile.open(path, std::ios::out | std::ios::trunc);
                    ...
                    for (int d2 = ghostsDim2; d2 < dim2 + ghostsDim2; d2++)
                        for (int d3 = ghostsDim3; d3 < dim3 + ghostsDim3; d3++)
                            for (int d4 = ghostsDim4; d4 < dim4 + ghostsDim4; d4++)
                                for (int d5 = ghostsDim5; d5 < dim5 + ghostsDim5; d5++)
                                {
                                    myfile << varname << "(" << d1 + 1 << "," << d2 + 1 << "," << d3 + 1 << "," << d4 + 1 << "," << d5 + 1 << ")="
                                           << std::scientific << std::setprecision(17) << field_5d[d1][d2][d3][d4][d5] << ";" << std::endl;
                                }
            }
            else
    ...
    /**
     * @brief Copies contents of real cells of another Hypercube5d o to real cells of this one.
     *
     * @param o
     */
    void Hypercube5d::copyRealFrom(Hypercube5d& o)
    {
        // TODO check ranges
        if (dim1 != o.dim1 || dim2 != o.dim2 || dim3 != o.dim3 ||
            dim4 != o.dim4 || dim5 != o.dim5)
        ...
        for (int k = ghostsDim3, kd = o.ghostsDim3; k < dim3 + ghostsDim3; k++, kd++)
            ...
            {
                (*this)[i][j][k][ii][jj] = o[id][jd][kd][iid][jjd];
            }
        // clang-format on
    }
