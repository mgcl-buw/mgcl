...
namespace mgcl
{
    ...
    void Blockstencil::updateGhosts(MPILevelData* mpiData, bool forceLocal, bool periodic)
    {
        // do nothing if single-gpu and Dirichlet bc's
        if (!periodic && (mpiData == nullptr || mpiData->mpiSize() == 1))
            return;
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf_ptr->field1d().data()), ghostsM * ngh * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->front[0], 0,
                           static_cast<void*>(rbuf_ptr->field1d().data()), ghostsM * ngh * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->back[0], 0,
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf_ptr->field1d().data()), ghostsM * ngh * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->back[0], 0,
                           static_cast<void*>(rbuf_ptr->field1d().data()), ghostsM * ngh * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->front[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf_ptr->field1d().data()), mgh * ghostsN * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->up[0], 0,
                           static_cast<void*>(rbuf_ptr->field1d().data()), mgh * ghostsN * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->down[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf_ptr->field1d().data()), mgh * ghostsN * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->down[0], 0,
                           static_cast<void*>(rbuf_ptr->field1d().data()), mgh * ghostsN * ogh * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->up[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf_ptr->field1d().data()), mgh * ngh * ghostsO * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->left[0], 0,
                           static_cast<void*>(rbuf_ptr->field1d().data()), mgh * ngh * ghostsO * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->right[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf_ptr->field1d().data()), mgh * ngh * ghostsO * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->right[0], 0,
                           static_cast<void*>(rbuf_ptr->field1d().data()), mgh * ngh * ghostsO * blocksize2 * stencilSize, MPI_DOUBLE, mpiData->left[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
            ...
            for (int gpj = getGhostsN(); gpj < getN() + getGhostsN(); gpj++)
                for (int gpk = getGhostsO(); gpk < getO() + getGhostsO(); gpk++)
                    for (size_t b = 0; b < getBlocksize(); b++)
                    {
                        ...
                        ret->getData()[b][gpi][gpj][gpk] = 1.0 / getData()[b][b][center][center][center][gpi][gpj][gpk];
                    }
