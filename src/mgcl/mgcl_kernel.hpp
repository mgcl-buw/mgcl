...
/**
 * Returns index of ghost cell corresponding to given cell index of a cuboid.
 ...
 * - m, n, o: cuboid dimensions without ghost cells
 * - mgh, ngh, ogh: cuboid dimensions including ghost cells
 * - ghosts_m, ghosts_n, ghosts_o: ghost cell count in each direction
 */
int get_ghost_idx_for_cell_idx(
    int idx,
    int i, int j, int k,
    int m, int n, int o,
...
/**
 * Copies content of buf_ghosts to ghost cells of buf_ghosts.
 * Data is stored consecutively in 3rd dimension, i.e. in 2d it would be
 *
 * 12345
 * 6...7 <- 123456789abcde
    ...
    int ghosts_m, int ghosts_n, int ghosts_o)
{
    ...
    if ((i < ghosts_m || i > mgh - 1 - ghosts_m) || (j < ghosts_n || j > ngh - 1 - ghosts_n) || (k < ghosts_o || k > ogh - 1 - ghosts_o))
    {
        buf_cuboid[idx] = buf_ghosts[get_ghost_idx_for_cell_idx(idx, i, j, k, m, n, o, mgh, ngh, ogh, ghosts_m, ghosts_n, ghosts_o)];
    }
})DELIM";
}