...
namespace mgcl
{
    ...
    void MultigridEngine::updateGhostsSeq(Cuboid& c, MPILevelData* mpiData, bool periodic, bool forceLocal)
    {
        ...
        // // Send data left
        // MPI_Sendrecv(&(c[ghosts_m][0][0]), 1, ghostsSliceX, mpiData->left[0], 0,
        //              &(c[0][0][0]), 1, ghostsSliceX, mpiData->right[0], 0, comm, MPI_STATUS_IGNORE);
        // // Send data right
        // MPI_Sendrecv(&(c[0][0][0]), 1, ghostsSliceX, mpiData->right[0], 0,
        //              &(c[ghosts_m + m][0][0]), 1, ghostsSliceX, mpiData->left[0], 0, comm, MPI_STATUS_IGNORE);

        // // Exchange data in y-direction
        // // Send data upwards
        // MPI_Sendrecv(&(c[0][ghosts_n][0]), 1, ghostsSliceY, mpiData->up[0], 0,
        //              &(c[0][0][0]), 1, ghostsSliceY, mpiData->down[0], 0, comm, MPI_STATUS_IGNORE);
        // // Send data downwards
        // MPI_Sendrecv(&(c[0][0][0]), 1, ghostsSliceY, mpiData->down[0], 0,
        //              &(c[0][ghosts_n + n][0]), 1, ghostsSliceY, mpiData->up[0], 0, comm, MPI_STATUS_IGNORE);

        // // Exchange data in z-direction
        // // Send data to the back
        // MPI_Sendrecv(&(c[0][0][ghosts_o]), 1, ghostsSliceZ, mpiData->back[0], 0,
        //              &(c[0][0][0]), 1, ghostsSliceZ, mpiData->front[0], 0, comm, MPI_STATUS_IGNORE);
        // // Send data to the front
        // MPI_Sendrecv(&(c[0][0][0]), 1, ghostsSliceZ, mpiData->front[0], 0,
        //              &(c[0][0][ghosts_o + o]), 1, ghostsSliceZ, mpiData->back[0], 0, comm, MPI_STATUS_IGNORE);
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf[0][0]), ghosts_m * ngh * ogh, MPI_DOUBLE, mpiData->front[0], 0,
                           static_cast<void*>(rbuf[0][0]), ghosts_m * ngh * ogh, MPI_DOUBLE, mpiData->back[0], 0,
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf[0][0]), ghosts_m * ngh * ogh, MPI_DOUBLE, mpiData->back[0], 0,
                           static_cast<void*>(rbuf[0][0]), ghosts_m * ngh * ogh, MPI_DOUBLE, mpiData->front[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf[0][0]), mgh * ghosts_n * ogh, MPI_DOUBLE, mpiData->up[0], 0,
                           static_cast<void*>(rbuf[0][0]), mgh * ghosts_n * ogh, MPI_DOUBLE, mpiData->down[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
        ...
        err = MPI_Sendrecv(static_cast<void*>(sbuf[0][0]), mgh * ngh * ghosts_o, MPI_DOUBLE, mpiData->right[0], 0,
                           static_cast<void*>(rbuf[0][0]), mgh * ngh * ghosts_o, MPI_DOUBLE, mpiData->left[0], 0,
                           mpiData->comm, MPI_STATUS_IGNORE);
        mgcl::mpi_util::mgclCheckMpiError(mpiData->comm, err, "MPI_Sendrecv");
