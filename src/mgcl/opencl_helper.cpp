...
namespace mgcl
{
    ...
    std::string OpenCLHelper::errorCodeToStr(cl_int errorCode)
    {
        switch (errorCode)
        {
        case CL_SUCCESS:
    ...
    void OpenCLHelper::mgcl_check_error(cl_int err, const char* operation, const char* filename, int line)
    {
        if (err != CL_SUCCESS)
        {
            std::string msg = "Caught OpenCL error '" + errorCodeToStr(err) + "' (code " + std::to_string(err) + ") during operation '" + operation + "', in " + filename + ":" + std::to_string(line);
